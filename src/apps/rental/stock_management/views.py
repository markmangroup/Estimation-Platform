...
class StockManagementListView(WarehouseViewMixin):
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        context['equipment_ids'] = StockAdjustment.objects.values_list('rental_product__equipment_id', flat=True).distinct()
        context['equipment_material_groups'] = StockAdjustment.objects.values_list('rental_product__equipment_material_group', flat=True).distinct()
        context['equipment_names'] = StockAdjustment.objects.values_list('rental_product__equipment_name', flat=True).distinct()
...
class SaveMassStockAdjustmentsView(View):
    def post(self, request, *args, **kwargs):
        try:
            ...
            return JsonResponse({
                'status': 'error',
                'message': f'Error saving stock adjustments: {str(e)}'
            })